...
finalize_filelist() {
    ...
    : > "$mf"
    awk -F'\t' -v mf="$mf" '!seen[$2]++ { print >> mf; n++; s+=$1 }
        END { printf "%.0f %.0f\n", n+0, s+0 }' \
...
PREV_B=0; PREV_T=0
render_progress() {
    local now dfiles=$2 dt sp pct eta e
    # awk OFMT (%.6g) prints large byte counts in scientific notation; bash $(( )) cannot parse that.
    printf -v now '%.0f' "${1:-0}" 2>/dev/null || now=0
...
printf -v TOTAL_FILES '%.0f' "$TOTAL_FILES" 2>/dev/null || TOTAL_FILES=0
printf -v TOTAL_SIZE '%.0f' "$TOTAL_SIZE" 2>/dev/null || TOTAL_SIZE=0
