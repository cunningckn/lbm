...
from lbm.feature_cache_paths import dataset_feature_path, verify_existing_cache
from lbm.feature_shards import file_sha256, write_sharded_cache
...
def main(argv=None):
    ...
    cache_parser.add_argument("--reuse-cache", action="store_true")
    cache_args, argv = cache_parser.parse_known_args(argv)
    explicit_output = any(item == "--output-dir" or item.startswith("--output-dir=") for item in argv)
    ...
    if not explicit_output and len(dataset.datasets) != 1:
        raise ValueError("dataset-local builds require one dataset; use --output-dir for a mixed cache")
    ...
    encoder = SimpleNamespace(
        config=cfg.model, camera_keys=cfg.model.camera_keys, img_backbone=build_vision_backbone(cfg.model)
    )
    ...
    for name in (
        ...
        "dataloader/pad.py",
        "history.py",
        "dataloader/custom/dataset.py",
        "utils/preprocess.py",
    ):
        preprocessing.update(name.encode())
        preprocessing.update((ROOT / "src/lbm" / name).read_bytes())
    metadata["extraction"] = dict(
        version=1,
        image_sizes={ds.spec.name: ds.image_size for ds in dataset.datasets},
        dtype=str(dtype),
        device_type=device.type,
        split=args.feature_split,
        preprocessing_sha256=preprocessing.hexdigest(),
        text_backbone_sha256=backbone_fingerprint(embedder.model),
        tokenizer_sha256=file_sha256(Path(cfg.clip.cache_dir) / cfg.clip.bpe_name),
    )
    if not explicit_output:
        cfg.output_dir = str(
            dataset_feature_path(dataset.datasets[0].root, cfg.model.vision_encoder, metadata, args.feature_split)
        )
    metadata["source_config"] = asdict(cfg)
    if Path(cfg.output_dir).exists():
        if not cache_args.reuse_cache:
            raise FileExistsError(f"feature cache already exists: {cfg.output_dir}; use --reuse-cache to verify it")
        verify_existing_cache(cfg.output_dir, metadata, len(dataset))
        print(f"feature cache verified: {cfg.output_dir} rows={len(dataset)}", flush=True)
        return cfg.output_dir

    def batches(start):
        loader = DataLoader(
            Subset(dataset, range(start, len(dataset))),
        ...
        with torch.no_grad():
            for raw in loader:
                ...
                batch.setdefault(
                    "camera_mask",
                    torch.ones(len(batch["state"]), len(cfg.model.camera_keys), dtype=torch.bool, device=device),
                )
    ...
    write_sharded_cache(
        cfg.output_dir, batches, rows=len(dataset), metadata=metadata, shard_rows=args.feature_shard_rows
    )
