"""CUDA graph helpers for fast LBM policy inference."""
...
from lbm.action_space import invert_action_space
from lbm.utils.preprocess import normalize, resize_pad_normalize, unnormalize
...
class FastInferenceGraph:
    ...
    def _copy_into(self, dst: torch.Tensor, array: np.ndarray) -> None:
        dst.copy_(torch.from_numpy(array).to(device=self.device, dtype=self.dtype))
    ...
    def _copy_inputs(self, obs: dict[str, Any], noise: np.ndarray | None) -> None:
        ...
        state = normalize(raw_state, self.policy.norm_stats["state"])
        self._copy_into(self.static_state, state[None])
        if noise is None:
            ...
            self._copy_into(self.static_noise, noise_arr)
        for cam in m.camera_keys:
    ...
    def _warmup_capture(self, replay_copy: Callable[[], None], replay_warmups: int) -> None:
    ...
    def _unnormalize_output(self) -> np.ndarray:
        assert self.output is not None
        actions_np = self.output[0].float().detach().cpu().numpy()
        chunk = unnormalize(actions_np, self.policy.norm_stats["actions"]).astype(np.float32)
        space = self.policy.action_space
        raw = getattr(self, "_last_raw_state", None)
        if space and raw is not None:
            chunk = invert_action_space(chunk, raw, space)
    ...
    def capture(
        ...
        self._warmup_capture(lambda: self._copy_inputs(warmup_obs, warmup_noise), replay_warmups)

    def infer(self, obs: dict[str, Any], noise: np.ndarray | None) -> np.ndarray:
        self._copy_inputs(obs, noise)
        self.graph.replay()
        return self._unnormalize_output()
...
class FastRTCInferenceGraph(FastInferenceGraph):
    ...
    def _copy_prefix(self, action_prefix: np.ndarray) -> None:
        ...
        self._copy_into(self.static_action_prefix, prefix[None])

    def _sample(self) -> torch.Tensor:
        return self.model.sample_actions_rtc(
            self.batch,
    ...
    def capture(
        ...
        self._warmup_capture(
            lambda: (self._copy_inputs(warmup_obs, warmup_noise), self._copy_prefix(warmup_prefix)),
            replay_warmups,
        )
